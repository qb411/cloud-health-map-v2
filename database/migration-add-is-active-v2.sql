...
-- Optimized version to avoid timeouts on large tables
-- Run this in your Supabase SQL Editor

-- Step 1: Add columns (fast - no data modification)
...
-- Step 2: Create indexes BEFORE updating data (improves update performance)
...
-- Step 3: Set default value for new rows only (doesn't update existing rows)
ALTER TABLE cloud_status
ALTER COLUMN is_active SET DEFAULT true;

-- Step 4: Update existing rows in small batches to avoid timeout
-- Update NULL is_active to true for rows with no end_time
UPDATE cloud_status
SET is_active = true
WHERE is_active IS NULL
  AND (end_time IS NULL OR end_time > NOW())
  AND id IN (
    SELECT id FROM cloud_status
    WHERE is_active IS NULL
    LIMIT 1000
  );

-- Update NULL is_active to false for rows with end_time in the past
UPDATE cloud_status
SET is_active = false
WHERE is_active IS NULL
...
-- Step 5: Update functions (lightweight - no data modification)
...
BEGIN
    ...
    WHERE provider = p_provider
      ...
      AND COALESCE(is_active, true) = true  -- Handle NULL as true during migration
      AND (end_time IS NULL OR end_time > NOW())
      AND created_at = (
...
-- Step 6: Create view for active incidents (handles NULL is_active gracefully)
...
SELECT
    ...
    created_at
FROM cloud_status
WHERE COALESCE(is_active, true) = true  -- Treat NULL as true during migration
  AND (end_time IS NULL OR end_time > NOW())
...
-- Step 7: Check migration progress
DO $$
DECLARE
    total_rows INTEGER;
    null_rows INTEGER;
    ...
    false_rows INTEGER;
BEGIN
    SELECT COUNT(*) INTO total_rows FROM cloud_status;
    SELECT COUNT(*) INTO null_rows FROM cloud_status WHERE is_active IS NULL;
    SELECT COUNT(*) INTO true_rows FROM cloud_status WHERE is_active = true;
    SELECT COUNT(*) INTO false_rows FROM cloud_status WHERE is_active = false;

    RAISE NOTICE 'Migration Progress:';
    RAISE NOTICE '  Total rows: %', total_rows;
    RAISE NOTICE '  NULL is_active: %', null_rows;
    RAISE NOTICE '  is_active = true: %', true_rows;
    RAISE NOTICE '  is_active = false: %', false_rows;

    IF null_rows > 0 THEN
        RAISE NOTICE '';
        RAISE NOTICE 'NOTE: % rows still have NULL is_active', null_rows;
        RAISE NOTICE 'Re-run the UPDATE statements in Step 4 to continue migration';
        RAISE NOTICE 'Or wait - new inserts will have is_active set automatically';
    ELSE
        RAISE NOTICE '';
        RAISE NOTICE 'Migration complete! All rows have is_active set.';
    END IF;
END $$;
